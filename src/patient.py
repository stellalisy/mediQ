...
class DirectPatient(Patient):
    def respond(self, question):
        ...
        response = get_response(messages, self.model_name, max_length=50).strip()
...
class InstructPatient(Patient):
    def respond(self, question):
        ...
        messages = [{"role": "system", "content": system_prompt}, {"role": "user", "content": user_prompt}]
        response = get_response(messages, self.model_name, max_length=50)[0]['generated_text']
        self.update_state(question, response)
        return response
...
class FactSelectPatient(Patient):
    def respond(self, question):
        if not self.facts:
            ...
            messages = [{"role": "system", "content": system_prompt}, {"role": "user", "content": user_prompt}]
            self.facts = get_response(messages, self.model_name, max_length=1000).splitlines()
